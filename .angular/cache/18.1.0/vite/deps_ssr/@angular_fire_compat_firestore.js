...
import {
  ...
  where
} from "./chunk-ROMFCG3J.js";
import "./chunk-P45GPCB4.js";
import {
  FirebaseApp
} from "./chunk-VJPXIWHH.js";
import {
  FIREBASE_APP_NAME,
  FIREBASE_OPTIONS,
  ɵapplyMixins,
...
import {
  ...
  ɵzoneWrap
} from "./chunk-K36N6YV2.js";
import {
  isPlatformServer
} from "./chunk-62Q7KYV5.js";
import {
  Inject,
  Injectable,
  InjectionToken,
  ...
  ɵɵdefineNgModule,
  ɵɵinject
} from "./chunk-JOLUHBXQ.js";
import {
  require_cjs
} from "./chunk-O4P5YJVT.js";
import "./chunk-EUY4BVVF.js";
import {
  firebase
...
function registerFirestore(instance) {
  ...
  instance.registerVersion(name2, version2, "node");
}
registerFirestore(firebase);

// node_modules/@angular/fire/fesm2022/angular-fire-compat-firestore.mjs
function _fromRef(ref, scheduler = import_rxjs3.asyncScheduler) {
...
function fromDocRef(ref, scheduler) {
  return fromRef(ref, scheduler).pipe((0, import_operators3.startWith)(void 0), (0, import_operators3.pairwise)(), (0, import_operators3.map)((snapshots) => {
...
function fromCollectionRef(ref, scheduler) {
  return fromRef(ref, scheduler).pipe((0, import_operators3.map)((payload) => ({
    payload,
    type: "query"
  })));
}
var AngularFirestoreDocument = class {
  ...
  valueChanges(options = {}) {
    return this.snapshotChanges().pipe((0, import_operators3.map)(({
      payload
   ...
   */
  get(options) {
    return (0, import_rxjs3.from)(this.ref.get(options)).pipe(keepUnstableUntilFirst);
  }
};
function docChanges(query2, scheduler) {
  return fromCollectionRef(query2, scheduler).pipe((0, import_operators3.startWith)(void 0), (0, import_operators3.pairwise)(), (0, import_operators3.map)((actionTuple) => {
...
function sortedChanges(query2, events, scheduler) {
  return docChanges(query2, scheduler).pipe(
    (0, import_operators3.scan)((current, changes) => combineChanges(current, changes.map((it) => it.payload), events), []),
    (0, import_operators3.distinctUntilChanged)(),
    // cut down on unneed change cycles
    (0, import_operators3.map)((changes) => changes.map((c) => ({
      type: c.type,
      payload: c
    })))
  );
...
var AngularFirestoreCollection = class {
  ...
  stateChanges(events) {
    ...
    if (events && events.length > 0) {
      source = source.pipe((0, import_operators3.map)((actions) => actions.filter((change) => events.indexOf(change.type) > -1)));
    }
    return source.pipe(
      // We want to filter out empty arrays, but always emit at first, so the developer knows
      // that the collection has been resolve; even if it's empty
      (0, import_operators3.startWith)(void 0),
      (0, import_operators3.pairwise)(),
      (0, import_operators3.filter)(([prior, current]) => current.length > 0 || !prior),
      (0, import_operators3.map)(([, current]) => current),
      keepUnstableUntilFirst
    );
  }
  /**
   ...
   */
  auditTrail(events) {
    return this.stateChanges(events).pipe((0, import_operators3.scan)((current, action) => [...current, ...action], []));
  }
  /**
  ...
  valueChanges(options = {}) {
    return fromCollectionRef(this.query, this.afs.schedulers.outsideAngular).pipe((0, import_operators3.map)((actions) => actions.payload.docs.map((a) => {
      if (options.idField) {
   ...
   */
  get(options) {
    return (0, import_rxjs3.from)(this.query.get(options)).pipe(keepUnstableUntilFirst);
  }
  /**
...
var AngularFirestoreCollectionGroup = class {
  ...
  stateChanges(events) {
    ...
    return docChanges(this.query, this.afs.schedulers.outsideAngular).pipe((0, import_operators3.map)((actions) => actions.filter((change) => events.indexOf(change.type) > -1)), (0, import_operators3.filter)((changes) => changes.length > 0), keepUnstableUntilFirst);
  }
  /**
   * Create a stream of changes as they occur it time. This method is similar to stateChanges()
   * but it collects each event in an array over time.
  ...
  valueChanges(options = {}) {
    ...
    return fromCollectionRefScheduled$.pipe((0, import_operators3.map)((actions) => actions.payload.docs.map((a) => {
      if (options.idField) {
    ...
    [this.firestore, this.persistenceEnabled$] = ɵcacheInstance(`${app.name}.firestore`, "AngularFirestore", app.name, () => {
      ...
      if (shouldEnablePersistence && !isPlatformServer(platformId)) {
        const enablePersistence = () => {
          try {
            return (0, import_rxjs3.from)(firestore.enablePersistence(persistenceSettings || void 0).then(() => true, () => false));
          } catch (e) {
            if (typeof console !== "undefined") {
              console.warn(e);
            }
            return (0, import_rxjs3.of)(false);
          }
        };
        return [firestore, zone.runOutsideAngular(enablePersistence)];
      } else {
        return [firestore, (0, import_rxjs3.of)(false)];
      }
